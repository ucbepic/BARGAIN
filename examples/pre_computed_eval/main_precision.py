...
for data in ['reviews', 'court_opinion', 'screenplay', 'wiki_talk', 'tacred', 'imagenet', 'onto', 'jackson']:
    ...
    for seed in tqdm(range(no_runs)):
        ...
        prism = PRISM_P(df['id'].to_numpy(), proxy, oracle, delta, target, k, seed=seed)
        est_positve = prism.process()

        curr_precision = precision(df['id'].to_numpy(), df['label'].to_numpy(), est_positve)
        res['precision'].append(curr_precision)
        res['recall'].append(recall(df['id'].to_numpy(), df['label'].to_numpy(), est_positve))
        res['orcale_usage'].append(oracle.get_number_preds())
        res['method'].append("PRISM_P_A")
        res['meets_target'].append(curr_precision>=target)
